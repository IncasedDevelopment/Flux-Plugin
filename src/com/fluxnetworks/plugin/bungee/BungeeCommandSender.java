...
import com.fluxnetworks.plugin.common.command.CommandSender;
...
public class BungeeCommandSender extends CommandSender {
	...
	public BungeeCommandSender(final net.md_5.bungee.api.CommandSender sender) {
		this.sender = sender;
		if (this.isPlayer()) {
			this.adventure = FluxPlugin.getInstance().adventure().player(((ProxiedPlayer) sender).getUniqueId());
		} else {
			this.adventure = FluxPlugin.getInstance().adventure().console();
		}
	}

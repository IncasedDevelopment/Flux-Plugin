...
public enum Config {
	...
	Config(final @NotNull String fileName){
		...
		final File dataFolder = FluxPlugin.getInstance().getDataFolder();
		if (!dataFolder.exists()) {
			dataFolder.mkdirs();
		}
		this.file = new File(FluxPlugin.getInstance().getDataFolder(), this.fileName);
	}

	public static void reloadAll() {
		final FluxPlugin plugin = FluxPlugin.getInstance();
